...
- **Bằng chứng trước & sau khi Containment**:
  - Log tấn công thành công (Trước khi contain): [`reports/attack-before.log`](file:///d:/VinAI_20K/Day24-Track2-Data-governance-security/reports/attack-before.log)
  - Log tấn công bị chặn (Sau khi contain): [`reports/attack-after.log`](file:///d:/VinAI_20K/Day24-Track2-Data-governance-security/reports/attack-after.log)
...
- **Trả lời**: 
  - Hệ thống đã cắt bỏ chân **Exfil Vector (`http_post`)** đối với các luồng xử lý dữ liệu restricted/PII và cô lập chân **Untrusted Content (`search_docs`)** không cho phép trực tiếp truy vấn dữ liệu **Private Data (`read_customer`)** dựa trên free text của attacker.
...
- **Trả lời**:
  1. **Control Trifecta Split ([`agent/runner.py`](file:///d:/VinAI_20K/Day24-Track2-Data-governance-security/agent/runner.py))**: Đứng vững hoàn toàn. Vì Run B chỉ đọc `customer_id` từ nguồn tin cậy (`related_tickets` trong `customers.json` gắn với `ticket_id` trích từ tên file), hoàn toàn bỏ qua mọi `customer_id` bị attacker cài cắm trong nội dung file `.md`.
  2. **Control PEP Policy Check ([`agent/policy.py`](file:///d:/VinAI_20K/Day24-Track2-Data-governance-security/agent/policy.py))**: Đứng vững hoàn toàn. Mọi yêu cầu egress (`http_post`) khi đang xử lý dữ liệu `restricted` đều bị chặn cứng (`decision=deny`).
  3. **Control Audit Ledger ([`agent/ledger.py`](file:///d:/VinAI_20K/Day24-Track2-Data-governance-security/agent/ledger.py))**: Đứng vững hoàn toàn. Mọi thao tác đều bị ghi lại trong chuỗi Hash Chain SHA-256 tamper-evident, attacker không thể sửa hay xóa log mà không bị phát hiện.
